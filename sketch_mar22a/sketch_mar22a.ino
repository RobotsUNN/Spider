...
public:
Leg(int fPin, int sPin, int tPin, float Direction,
   int cMeshOffset, int fMeshOffset, int tMeshOffset, Point& cPosition)
{
  _cPWMServoPin = fPin;
  _fPWMServoPin = sPin;
  _tPWMServoPin = tPin;
...
{//всё в милиметрах
  ...
  {
    fAngle = 180 -femurPrimaryAngle - this->_fMeshOffset;
  }
  ...
  if (this->_Direction == - 1)
  {
    tAngle = 180 - (this->_tMeshOffset+femurPrimaryAngle-(primaryTibiaAngle));
  }
...
Leg second(0,1,2,-1,130,48,5,cPos);
Point f(-150,-20,0);
Point s(-120,-20,0);
Point t(-120,-50,0);
Point fo(-150,-50,0);
